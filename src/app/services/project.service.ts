...
export class ProjectService {
  ...
  /**
   * Gets projects by collaborator id
   * @param filter Filter object with status array and/or createdBy
   * @returns Observable with the list of projects
   */
  getProjectsByCollaboratorId(filter: any): Observable<ApiResponse<any[]>> {
    return this.http.post<any>(`${this.apiUrl}/filter-by-collaborator`, { ...filter, username: 'walter.bates', password: 'bpm'}, this.httpOptions)
      .pipe(
        map(response => {
          return {
            success: true,
        ...
        catchError(error => {
          ...
          return [{
            success: false,
            error: errorMessage,
            message: 'Error al obtener los proyectos por colaborador'
          }];
        })
      );
  }
